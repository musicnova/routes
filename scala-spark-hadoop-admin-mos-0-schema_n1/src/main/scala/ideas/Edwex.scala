...
object Edwex extends Serializable {
  ...
  def edwexAll(): String = {
    ...
    writer.writeRow(Seq("0", "1", "FTASK", "N", "NUMERIC", "n/c"))
    writer.writeRow(Seq("0", "2", "FTASK", "LEVEL", "NUMERIC", "n/c"))
    writer.writeRow(Seq("0", "3", "FTASK", "NAME", "VARCHAR65535", "n/c"))
    writer.writeRow(Seq("0", "4", "FTASK", "LOGIC", "VARCHAR65535", "n/c"))
    writer.writeRow(Seq("0", "5", "FTASK", "SRC_FIELD", "VARCHAR65535", "n/c"))
    writer.writeRow(Seq("0", "6", "FTASK", "SRC_SYSTEM", "VARCHAR65535", "n/c"))
    writer.writeRow(Seq("0", "7", "FTASK", "SRC_LOAD", "VARCHAR65535", "n/c"))
    writer.writeRow(Seq("0", "8", "FTASK", "SRC_TABLE", "VARCHAR65535", "n/c"))
    writer.writeRow(Seq("0", "9", "FTASK", "KHD_TABLE", "VARCHAR65535", "n/c"))
    writer.writeRow(Seq("0", "10", "FTASK", "REGION", "VARCHAR65535", "n/c"))
    writer.writeRow(Seq("0", "11", "FTASK", "COMMENT", "VARCHAR65535", "n/c"))
  ...
  def taskAll(): String = {
    ...
    // 1 to 58
    writer.writeRow(Seq("1",  "2", "***CASE_OWNERS.CODE", "(select 1 as CODE) dual", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "CaseOwnersCode {\n    val TROYKA_WIFI_MATCH = 1\n    val PARKING_WIFI_MATCH = 2\n    val EXAMPLE = 3\n  }"))
    writer.writeRow(Seq("2",  "2", "CASE_OWNERS.STAMP", "(select CAST(TO_CHAR(NOW(), 'YYMMDDHH24MI') as bigint) * 100 + 1 as STAMP, 1 as CODE) dual", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "-"))
    writer.writeRow(Seq("3",  "2", "CASE_OWNERS.ID", "(select 1 as ID, 1 as CODE) dual", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "-"))
    writer.writeRow(Seq("4",  "2", "CASE_OWNERS.SCHEMA", "(select 1 as SCHEMA, 1 as CODE) dual", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "object CaseOwnersSchema {\n    val METROS_TRANSFERS_ENTRANCES = 1\n    val CAMERAS_CROSSROADS_PARKINGS = 2\n  }"))
    ...
    writer.writeRow(Seq("6",  "2", "***CASE_RECORDS.ID", "(select (select max(STAMP) from v001_o1_case_owners where schema = 1) * 1000000 + generate_series as ID from generate_series(1000001, 9999999)) dual", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "-"))
    ...
    writer.writeRow(Seq("8",  "2", "CASE_RECORDS.TYPE", "(select 1 as TYPE, (select max(STAMP) from v001_o1_case_owners where schema = 1) * 1000000 + generate_series as ID from generate_series(1000000, 4999999) union all select 3 as TYPE, (select max(STAMP) from v001_o1_case_owners where schema = 1) * 1000000 + generate_series as ID from generate_series(5000000, 9999999)) dual", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "object CaseRecordsType {\n    val STATION = 1\n    val EXIT = 2\n    val ENTRANCE = 3\n    val CAMERA = 4\n    val CROSSROAD = 5\n    val PARKINGS = 6\n  }"))
    ...
    writer.writeRow(Seq("34", "2", "CASE_GRAPHS.LINK_TYPE", "SELECT", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "object CaseGraphsLinkType {\n    val MOVEMETRO = 1\n    val TRANSMETRO = 2\n    val ENTERMETRO = 3\n    val MOVECAR = 4\n  }"))
    writer.writeRow(Seq("35", "2", "CASE_GRAPHS.COLOR", "SELECT", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "object CaseGraphsColor {\n    val UNKNOWN = 0\n    val HFF0000 = 1\n    val H00FF00 = 2\n    val H0000FF = 3\n  }"))
    ...
    writer.writeRow(Seq("39", "2", "CASE_STATS.RESOURCE_TYPE", "SELECT", "n/a", "data.mos.ru", "000001", "dual", "dual", "mos", "object CaseStatsResourceType {\n    val DEMO = 1\n    val YANDEX_API = 2\n  }"))
  ...
  def main(args: Array[String]): Unit = {
    ...
    writeToFile("/home/user/CODE/ftask.csv", taskAll())
  }
}
